  ...
  <div class="view">
    <Panel :title="$t('m.User_User') ">
      <div slot="header">
        ...
        <!-- แถบ selection info -->
        <div v-if="(selectedUserIDs.length && total) || selectAllAcross" class="selection-info">
        ...
        <el-table-column label="Group">
          <template slot-scope="scope">
            <template v-if="scope.row.groups && scope.row.groups.length">
              <el-tag v-for="(g, i) in scope.row.groups"
                      :key="g + i"
                      size="mini"
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item :label="$t('m.Two_Factor_Auth')">
              <el-switch v-model="user.two_factor_auth" :disabled="!user.real_tfa"></el-switch>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Open Api">
              <el-switch v-model="user.open_api"></el-switch>
            </el-form-item>
          </el-col>
          <el-col :span="8">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="danger"
                   ...
                   @click="removeFromGroup">Remove from group</el-button>
...
export default {
  ...
  methods: {
    ...
    // delete (supports across pages like problems)
    async handleDeleteClick () {
      if (!this.selectAllAcross &&
        ...
        try {
          await this.$confirm(
            `You selected all ${this.userList.length} users on this page. Delete ALL ${this.total} results matching this search?`,
            'Delete Users',
            { type: 'warning' }
          )
        } catch (e) { return }
      ...
      if (this.selectAllAcross) {
        ...
        try {
          ...
          for (let i = 0; i < allIds.length; i += chunk) {
            ...
            await api.deleteUsers(batch.join(','))
          }
    ...
    async confirmGroup () {
      ...
      if (this.selectAllAcross) {
        ...
        ids = await this.fetchAllMatchingUserIds()
      } else {
      ...
      try {
        ...
        for (let i = 0; i < ids.length; i += chunk) {
          ...
          await api.assignUsersToGroup({ user_ids: batch, group_name })
        }
        this.$success('Saved')
        this.clearSelection()
    ...
    async removeFromGroup () {
      ...
      try {
        ...
        for (let i = 0; i < ids.length; i += chunk) {
          const batch = ids.slice(i, i + chunk)
          await api.removeUsersFromGroup({ user_ids: batch, group_name })
        }
    ...
    async clearAllGroups () {
      ...
      try {
        ...
        for (let i = 0; i < ids.length; i += chunk) {
          const batch = ids.slice(i, i + chunk)
          await api.clearUsersGroups({ user_ids: batch })
        }
...
.import-user-icon { color: #555555; margin-left: 4px; }
